...
def generate_holiday_period(start_date: datetime, periods: int) -> list:
    return pd.date_range(start_date, periods=periods).to_pydatetime().tolist()


def generate_season_ind(
    dates,
    style: Literal[
        "fixed+error",
        "semi-markov",
    ...
    # not implemented: SARIMA style season

    n_periods = len(dates)

    if style == "fixed+error":
        # generates an indicator array of length n_periods with a seasonality of period_length and disturbance stdev of period_sd
        # seasonality type: fixed + error
        # result is a non-uniform distribution of weekdays in the histogram (if frequency is weekly, as an example)

        events = np.zeros(n_periods)
        event_inds = np.arange(n_periods // period_length + 1) * period_length + start_index
        disturbance = np.round(
            np.random.normal(
                loc=0.0,
                scale=period_sd,
                size=len(event_inds),
        ...
        # print(disturbance)
        # print(event_inds)
        # weekdays = np.mod(event_inds,period_length)
        # print(np.histogram(weekdays, bins=np.arange(period_length)))
        return events

    elif style == "semi-markov":
        # generates an indicator array of length n_periods with a seasonality of period_length and disturbance stdev of period_sd
        # seasonality type: semi-Markov
        # result is a uniform distribution of weekdays (if frequency is weekly, as an example)

        events = np.zeros(n_periods)
        event_inds = [start_index]
        new = np.round(np.random.normal(loc=period_length, scale=period_sd, size=1))[0]
        ...
        event_inds = np.array(event_inds).astype(int)
        events[event_inds] = 1
        # print(event_inds)
        # weekdays = np.mod(event_inds,period_length)
        # print(np.histogram(weekdays, bins=np.arange(period_length)))
        ...
        # generates an indicator array of length n_periods with a seasonality of period_length and disturbance stdev of period_sd
        # seasonality type: holiday
        # here we just work with calendar holidays (Easter+Christmas week as an example)
        season_dates = (
            generate_holiday_period(
        ...
        events = np.where([x in season_dates for x in dates], 1, 0)
        return events
    ...
    elif style == "sin_covar":
        covar_wave = (np.sin((np.arange(n_periods) - start_index) / period_length / 6 * np.pi) + 2) / 2
        dx = np.full_like(covar_wave, 0.4)
        sin_wave = np.sin((covar_wave * dx).cumsum())
        return sin_wave


def generate_polynom_trend(interpol_nodes, n_periods: int):
    ...
    if len(interpol_nodes) == 0:
        ...
        # No trend component:
        trend = np.zeros(n_periods) + interpol_nodes[0][1]
        return trend
...
def generate_ARMA(
    ...
    u = np.random.normal(loc=0.0, scale=rndwalk_scale, size=n_periods)  # noise
    ts = np.zeros(n_periods)  # X_t
...
def create_time_series(
    ...
    AR: list = [1],
    MA: list = [],
    rndwalk_scale: float = 2,
    exogenous: list = [],
    season_conf: dict[str, str] = {"style": "holiday"},
    ...
    # Date index:
    dates = pd.date_range(start_date, periods=n_periods).to_pydatetime().tolist()

    # Cubic trend component:
    trend = generate_polynom_trend(interpol_nodes, n_periods)

    # Structural break:
    break_effect = np.zeros(n_periods)
    for b in level_breaks:
        break_effect[b[0] :] += b[1]

    # ARMA(AR,MA) component:
    rndwalk = generate_ARMA(
        AR=AR,
        MA=MA,
        rndwalk_scale=rndwalk_scale,
        n_periods=n_periods,
        exogenous=exogenous,
    )
    ...
    if season_conf is not None:
        season = generate_season_ind(dates=dates, **season_conf)
    ...
    if len(man_outliers) == 0:
        outliers = np.ones(n_periods)
    else:
        outliers = np.asarray(man_outliers)
    ...
    df = pd.DataFrame(
        list(
            zip(
                dates,
                (trend + break_effect + rndwalk + noise) * season * outliers,
            )
        ),
        index=dates,
        columns=["Date", "Value"],
